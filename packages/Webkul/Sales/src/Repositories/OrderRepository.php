...
class OrderRepository extends Repository
{
    ...
    public function create(array $data)
    {
        ...
        try {
            ...
            $json= json_decode($data['customer']);
            //File::append(storage_path('logs/C-'.Carbon::now()->format('Y-m-d').'.log'),$json->an8);//JDE
           ...
           // return $data['items'];
            foreach ($data['items'] as $item) {
               ...
               // File::append(storage_path('logs/X-'.Carbon::now()->format('Y-m-d').'.log'),$orderItem);//JDE


               
                array_push($detail, [//JDE
                        'producto' =>  strtoupper($orderItem->sku),//JDE
                         'cantidad' =>  $orderItem->qty_ordered//JDE
                    ]);//JDE
            ...
            if($BSSV=='ON'){
                $JDE = (new SoapJdeController)->CrearOrdenJDE($json->an8,$detail,$order->id);//JDE
                if( $JDE == '500'){
                    DB::rollBack();
                    abort(500, 'Orden no se pudo generar.');
                }
            }
            //File::append(storage_path('logs/O-'.Carbon::now()->format('Y-m-d').'.log'),$order);//JDE
