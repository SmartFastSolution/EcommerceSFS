...
class SubscriptionController extends Controller
{
    ...
    public function points(Request $request)
    {
     //   $this->validate(request(), [
       //     'sub_mail' => 'email|required',
       // ]);

        $email = $request->sub_mail;
        $phone = $request->sub_cel;
        $country = $request->sub_ciudad;
      //  $email = request()->input('subscriber_email');

     $validation =  DB::table('points')->where('email' , $email)->where('phone' , $phone)->first();
        if( !isset( $validation)){
            ...
            session()->flash('success', 'Gracias por tus registro.');

            return redirect()->back();
        }else{
            session()->flash('error', 'Ya estas registrado.');
            return redirect()->back();
        }
