...
abstract class AbstractType
{
    ...
    public function getPriceHtml()
    {
        if ($this->haveSpecialPrice()) {
            ...
            //'<div class="sticker sale">' . trans('shop::app.products.sale') . '</div>' . 
                 // '<div class="sticker sale" style="color:green">' . "Oferta" . '</div>' . 
             //     '<span class="regular-price" style="color:red;text-decoration:line-through;">' . core()->currency($this->product->price) . '</span>'
              //  . '<span class="special-price">' . core()->currency($this->getSpecialPrice()) . '</span>';
               // '<div>' . "Oferta" . '</div>' . 
                '<label>' . core()->currency($this->getSpecialPrice()) . '</label>'.
                '<span style="text-decoration:line-through;">' . core()->currency($this->product->price).   '<span/>'  ;
        } else {
            //$html = '<span>' . core()->currency($this->product->price) . '</span>';
            $html = '<label>' . core()->currency($this->product->price) . '</label>';
        }
    ...
    public function getDiscountHtml()
    {
        if ($this->haveSpecialPrice()) {
            ...
            $special_price= floatval($this->getSpecialPrice());
            $percent =   ( $price - $special_price ) / $price ;

            $html = '<label>' .number_format($percent*100, 0). '</label>';
