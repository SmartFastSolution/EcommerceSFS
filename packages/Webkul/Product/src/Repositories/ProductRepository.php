...
class ProductRepository extends Repository
{
    ...
    public function getProductsRelatedToCategory($categoryId = null)
    {
        ...
        $childrens=DB::table('categories')->select('id')->where('parent_id',$categoryId)->get();
        ...
        if ($categoryId) {
            $qb  ->whereIn('product_categories.category_id',  $array)
                 ->groupBy('product_id');
            //->where('product_categories.category_id',$categoryId);
        }

       // return $qb->get();
        return $qb->paginate(1);
    }
     ...
     * @return \Illuminate\Support\Collection
     */
    public function getProductsRelatedToCategoryCode($code = null)
    {

        $db=DB::table('categories')->select('id')->where('code',$code)->first();

        

        if($db){
            $childrens=DB::table('categories')->select('id')->where('parent_id',$db->id)->get();
            $array = array($db->id);
            foreach($childrens as $children){
                array_push($array, $children->id);
            }
        ...
        if ($db->id) {
           $qb
           //->where('product_categories.category_id',$db->id);
           ->whereIn('product_categories.category_id',  $array)
           ->groupBy('product_id');
            //$qb->where('categories.code',$id);
        }

        }
       
       // return $qb->get();
        return $qb->paginate(25);
    }
    ...
    public function getDealsProducts()
    {
        ...
        $results = app(ProductFlatRepository::class)->scopeQuery(function ($query) {
            ...
            return $query->distinct()
                ...
                ->where('product_flat.status', 1)
                ->where('product_flat.visible_individually', 1)
                ->whereBetween('product_flat.special_price_from', [$weekStartDate, $weekEndDate])
                ->whereNotNull('product_flat.special_price')
                ->where('product_flat.channel', $channel)
                ->where('product_flat.locale', $locale)
                ->inRandomOrder();
 ...
 /**
     ...
     */
     ...
     public function allproducts_relationcategory($id){

     // Funcion para buscar todos los productos relacionados por la categoria del producto seleccionado
     // CErtifsa Tyrone
    

       $categoryinproduct = DB::table('product_categories')
       ->select(
      //     'products.sku',
       'categories.id', 
      // 'category_translations.name'
       ...
       ->where('category_translations.locale','=','es')
       ->get();

       $cat=$categoryinproduct->toArray();
        $arr = [];
        foreach($categoryinproduct as $row)
        {
            $arr[] = (array) $row;
        }
       $productosallcotegory = DB::table('product_categories')
       ->select('product_categories.product_id', 'products.sku', 'product_flat.name','product_images.path')
       ->join('categories','categories.id','=','product_categories.category_id')
       ->join('category_translations','category_translations.category_id','=','categories.id')
       ->join('products','products.id','=','product_categories.product_id')
       ->join('product_flat','product_flat.product_id','=','products.id')
       ->join('product_images','product_images.product_id','=','products.id')
       //->WhereIn( 'product_categories.category_id',[57, 58, 59,770])
       ->WhereIn( 'product_categories.category_id', $arr)
       ->distinct()
       ->get();

     /// dd($productosallcotegory);
    return $productosallcotegory;
    }
}