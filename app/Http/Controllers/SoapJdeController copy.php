...
//CREAR CLIENTE EN JDE
    public function CrearClienteJDE($nombre,$identificacion,$company,$correo,$tipo_identificacion){
        ...
        $header='
        <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:orac="http://oracle.e1.bssv.JP010020/">
            <soapenv:Header>
        ...
        $body_recibido='<soapenv:Body>
            <orac:processCustomerV2>
            <customer>
	            <company>'.$company.'</company>
            </customer>
            <distributionV2>
	            <glOffSet>CNAL</glOffSet>
	        </distributionV2>
            <invoice>
	            <currencyCode>USD</currencyCode>
	            <paymentInstrumentCode>E</paymentInstrumentCode>
	            <paymentTermsCode>015</paymentTermsCode>
	        </invoice>
            <processAddressBookV2>
                <addressBook>
                    <entity>
                        <entityLongId>'.$identificacion.'</entityLongId>
                        <entityTaxId>'.$identificacion.'</entityTaxId>
                    </entity>
                    <businessUnit>1</businessUnit>
                    <entityName>'.$nombre.'</entityName>
                    <entityTypeCode>'.$tipo_identificacion.'</entityTypeCode>
                    <personCorporationCode>'.$tipo_identificacion.'</personCorporationCode>
                    <processing>
            ...
            <tax>
                ...
                <taxExplanationCode>V</taxExplanationCode>
                <taxRateAreaCode>VENTAS</taxRateAreaCode>
            </tax>';
        $footer='</orac:processCustomerV2>
            </soapenv:Body>
        </soapenv:Envelope>';
        ...
        File::append(storage_path('logs/CLIENT-SOAP-'.Carbon::now()->format('Y-m').'.log'),PHP_EOL."
        _________________________________________________________________________________________________________________________"
        .PHP_EOL);

        File::append(storage_path('logs/CLIENT-SOAP-'.Carbon::now()->format('Y-m').'.log'),$request_xml.PHP_EOL);

        File::append(storage_path('logs/CLIENT-SOAP-'.Carbon::now()->format('Y-m').'.log'),PHP_EOL."
        _________________________________________________________________________________________________________________________"
        .PHP_EOL);
        File::append(storage_path('logs/CLIENT-SOAP-'.Carbon::now()->format('Y-m').'.log'),$client->responseData);
        //echo "WSDL: ".$wsdl.PHP_EOL;
        //echo "response2: ".$client->responseData.PHP_EOL;
        //echo "error: ".$client->getError().PHP_EOL;
